...
fn ContactsView() -> impl IntoView {
	section().class("p-8 max-w-2xl mx-auto").child((
		...
		div().class("mt-12").child((
			h2().class("text-2xl font-bold mb-4 text-center").child("Source Code"),
			a()
				.attr("href", "https://github.com/valeratrades/site")
				.attr("target", "_blank")
				.attr("rel", "noopener noreferrer")
				.class("block p-4 bg-gray-800 hover:bg-gray-700 rounded-lg transition-colors")
				.child(
					img()
						.attr("src", "https://gh-card.dev/repos/valeratrades/site.svg?fullname=")
						.attr("alt", "valeratrades/site GitHub repository")
						.class("w-full"),
				),
		)),
	))
